...
const lakeList = document.getElementById('lakeList');
...
searchBar.addEventListener('keyup', (e) => {
    ...
    const filteredInfo = lakeInfo.filter((lake) => {
        return (
            lake.date.toLowerCase().includes(searchString)
        );
    });
    displayLAke(filteredInfo);
});

const loadLake = async () => {
    try {
        const res = await fetch('oroville.js');
        lakeData = await res.json();
        displayLake(lakeData);
    } catch (err) {
        console.error(err);
    }
};

const displayLake = (lakeData) => {
    const htmlString = lake
        .map((lake) => {
            return `
            <li class="lake">
                <h2>${lake.date}</h2>
